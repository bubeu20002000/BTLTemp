...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE: //khi kích thước cửa sổ chính của chương trình thay đổi mới chạy vào phần này, khi mới chạy chương trình cũng nhảy vào đây
	{
		...
		if (exist == 0) {
			...
			MoveWindow(hWnd, 200, 150, 1350, 600, TRUE);//200, 150, 990, 600
			exist = 1;
		}
    ...
    case WM_COMMAND:
        {
			...
			case ID_OPTION_FONT: {
				...
				if (ChooseFont(&cf))
				{
					...
					{
						g_hfFont = hf;
						SendDlgItemMessage(hWnd, ID_TEXTBOX, WM_SETFONT, (WPARAM)g_hfFont, TRUE);
					}
					else
					{
			...
			case ID_FILE_SAVE:
				...
				break;
			case IDM_FILE_NEW: 
				hwndDrawArea = CreateWindow(TEXT("static"), TEXT("Draw"),
					WS_VISIBLE | WS_CHILD | SS_GRAYFRAME | BS_BITMAP
					, 5, 165, 1125, 340, hWnd, (HMENU)ID_DRAW_AREA, NULL, NULL);
				break;
			case ID_FILE_OPEN:
			{
				OpenImage();
    ...
    case WM_PAINT:
        {
			...
			Gdiplus::Bitmap bmp(L"mainjava.png");
			gf.DrawImage(&bmp, 150, 150);
			EndPaint(hwndDrawArea, &ps);*/
